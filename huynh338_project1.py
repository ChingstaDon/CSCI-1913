...
class Mumble():
    ...
    def make(self, max):
        ...
        while(max > 0):
            ...
            count = 2
            if(len(word) == 0):
                R = self.__ran_num.choose(choosers[' '][0])
                while(R > 0):
                    R -= choosers[' '][count]
                    if(R <= 0):
                        word += choosers[' '][count - 1]
                    count += 2
            else:
                R = self.__ran_num.choose(choosers[word[-1]][0])
                while(R > 0):
                    R -= choosers[word[-1]][count]
                    if(R <= 0):
                        if(choosers[word[-1]][count - 1] == ' '):
                            break
                        word += choosers[word[-1]][count - 1]
                    count += 2
            max -= 1
        return word
